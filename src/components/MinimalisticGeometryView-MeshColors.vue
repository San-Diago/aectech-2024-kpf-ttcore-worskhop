...
async function compute() {
    ...
    if (doc.metadata) {
        console.log("Metadata received:", doc.metadata);
        emits("updateMetadata", doc.metadata);
    }

    // Extract color information from metadata
    let vertexColors = [];
doc.metadata.forEach(item => {
    if (item.name === "vertexColors") {
        // Extract RGB from the value, normalize and add to the vertexColors array
        const rgb = item.value.split(',').map(Number).map(val => val / 255); // Normalize RGB to 0-1 range
        if (rgb.length === 3) {
            vertexColors.push(new THREE.Color(rgb[0], rgb[1], rgb[2]));
        }
    }
});
    ...
    scene.traverse((child) => {
        if (!child.isLight && child.isCompute) {
            console.log("Removing child from scene:", child);
            scene.remove(child);
        }
    });
    ...
    loader.parse(buffer, function (object) {
        console.log("Parsed object from Rhino Compute:", object);
        object.isCompute = true;
        object.castShadow = true;

        let meshIndex = 0; // Initialize a counter for the mesh index
...
object.traverse((child) => {
    if (child.isMesh) {
        ...
        // Assign color if vertexColors are available and valid
        if (vertexColors.length > 0 && meshIndex < vertexColors.length) {
            const color = vertexColors[meshIndex];
            child.material = new THREE.MeshBasicMaterial({
                color: color,
                side: THREE.DoubleSide
            });
            console.log(`Assigning color to mesh ${meshIndex}:`, color); // Debug log
            meshIndex++; // Increment the mesh counter after assigning color
        } else {
            // Use default material with texture
        ...
        scene.add(object);
        console.log("Compute done, object added to scene");
    });
}
